...
namespace NSS_3330S
{
    public class DBExEventData : SQLiteBase
    {
        public string strTableName = "";

        public DBExEventData()
        {
            // 테이블 명
        ...
        public EventProperties[] SelectEventDataInRange(DateTime dtStart, DateTime dtEnd, string col_name = "Time", int nMaxCount = 10000)
        {
                    ...
                    using (SQLiteCommand sqCommend = new SQLiteCommand(SQConn))
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            for (int nCnt = 0; nCnt < dt.Rows.Count; nCnt++)
                            {
                                ...
                                colInfo[nCnt].Section = dr.ItemArray[1].ToString();
                                colInfo[nCnt].Detail = dr.ItemArray[2].ToString();
                            }
                        }
        ...
        public DataTable DataTableSelectEventDataInRange(DateTime dtStart, DateTime dtEnd, string col_name = "Time", int nMaxCount = 10000)
        {
            using (SQLiteConnection SQConn = new SQLiteConnection())
            {
                SQConn.ConnectionString = connecString;
