...
namespace NSS_3330S
{
    public class DBExStripLog : SQLiteBase
    {
        public string strTableName = "TRK_STRIP_LOG";
            ...
            using (SQLiteConnection SQConn = new SQLiteConnection())
            {
                ...
                using (var transaction = SQConn.BeginTransaction())
                {
                    ...
                    using (SQLiteCommand sqCommend = new SQLiteCommand(SQConn))
                    {
                        ...
                        sqCommend.CommandType = CommandType.Text;
                        //select query
                        sqCommend.CommandText = string.Format("SELECT Time, LotId, StripId  FROM {0} WHERE {1} = '{2}'", TABLE_NAME, StripProperties.ColName.LotId.ToString(), strLotId);
        ...
        //{
        //    DeleteDB_TrigReset();
        //    DeleteDB_TrigSet(nPeriod, StripProperties.ColName.Time.ToString());
        //}
        #endregion
    }
}
