...
{
    public class MnPickerZeroSet_Pickup : SeqBase
    {
        int m_nHeadNo = 0;
        int m_nStageNo = 0;
        ...
        int MovePickerZeroSetPosXY(long lDelay = 0)
        {
            ...
            //속도 참고용  POSDF 설정
            nPosArray[0] = POSDF.CHIP_PICKER_CHIP_UNLOADING_GOOD_REF_T1;
            nPosArray[1] = POSDF.MAP_PICKER_STRIP_UNLOADING_STAGE_1;

            nAxisArray[0] = (int)SVDF.AXES.CHIP_PK_1_X + m_nHeadNo;
            nAxisArray[1] = (int)SVDF.AXES.MAP_STG_1_Y + m_nStageNo;
        ...
        void UpdateTempData()
        {
            for (int nPadNo = 0; nPadNo < CFG_DF.MAX_PICKER_PAD_CNT; nPadNo++)
            {
                ...
                {
                    //칩 두께만큼 더해준다.
                    ConfigMgr.Inst.TempCfg.OffsetPnP[m_nHeadNo].dPnpOffsetZ[nPadNo].dPickUp[m_nStageNo] = Math.Round(m_dResultPosZ[nPadNo], 2);// + RecipeMgr.Inst.TempRcp.MapTbInfo.dChipThickness;
                }
            }
        }
    }
