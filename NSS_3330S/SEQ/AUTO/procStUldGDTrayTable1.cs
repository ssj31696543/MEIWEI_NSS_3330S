                ...
                case 10:
                    {
                        ...
                        break;
#endif
                        if (!SafetyCheckTrayTableMove(m_nTableNo, POSDF.TRAY_STAGE_TRAY_UNLOADING)) 
                            return;
                    }
                    break;
                // 테이블 준비위치 이동
                ...
                case 21:
                    {
                        ...
                        //if (GbVar.Seq.bStopSupplyTray == true)
                        //{
                        //    LeaveCycle();
                        //    return;
                        //}
...
#region 트레이를 공급 받고 난 후 스테이지 다운 -> 작업 위치 이동 -> 스테이지 업 -> 작업 시작 -> 트레이 만재 대기 (case 30 ~)
                ...
                case 42:
                    {
                        if (m_bFirstSeqStep)
                        {
                            ...
                            //20230108 CHOH: 트레이 스테이지가 워킹포지션에서 대기하도록 수정
                            //if (GbVar.bTrayStageTest)
                            //{
                            //    if (IsInWorkReadyPosGdTrayStgY(m_nTableNo, 0))
                            //    {
                            //        break;
                            //    }
                            //}
                        ...
                        //if (GbVar.bTrayStageTest)
                        //{
                        //    nFuncResult = MoveWorkReadyPosGdTrayStgY(m_nTableNo, 0);
                        //}
                        //else
                        //{
                            nFuncResult = MovePosGdTrayStgY(m_nTableNo, POSDF.TRAY_STAGE_TRAY_WORKING_P1);
                ...
                case 46:
                    {   
                        ...
                        //20230613 choh: 다른스테이지 작업중 비트 확인 안하도록 수정
                        {
                            //기존소스
                            //// 다른 스테이지가 작업 중일 때
                            //if (IsWorkSideStage()) return;
                        }
                ...
                case 48:
                    {
                        ...
                        if (FNC.IsSuccess(nFuncResult))
                        {
                            SeqHistory("Tray Table Cylinder Up Case 48", m_strTableName, "Complete");
                        }
                    }
                    break;
                ...
                case 60:
                    {
                        ...
                        GbVar.Seq.bTrayUnitMisPlace[CFG_DF.TRAY_STG_GD1] = false;
                        GbVar.Seq.sUldGDTrayTable[m_nTableNo].bSeqIfVar[seqUldGoodTrayTable.TRAY_VISION_START] = true;
                ...
                case 70:
                    {
                        ...
                        if (GbVar.Seq.bTrayUnitMisPlace[CFG_DF.TRAY_STG_GD1])
                        {
                            GbVar.Seq.bTrayUnitMisPlace[CFG_DF.TRAY_STG_GD1] = false;
                        ...
                        if (nFuncResult == FNC.SUCCESS)
                        {
                            ...
                            for (int nRow = 0; nRow < RecipeMgr.Inst.Rcp.TrayInfo.nTrayCountY; nRow++)
                            {
                                ...
                                {
                                    if (GbVar.Seq.sUldGDTrayTable[CFG_DF.TRAY_STG_GD1].Info.UnitArr[nRow][nCol].IS_UNIT &&
                                        GbVar.Seq.sUldGDTrayTable[CFG_DF.TRAY_STG_GD1].Info.UnitArr[nRow][nCol].TRAY_MEASURE_CODE != 0)
                                    {
                                        GbVar.Seq.bTrayUnitMisPlace[CFG_DF.TRAY_STG_GD1] = true;
...
#region 트레이 배출 (case 90 ~)
                ...
                case 104:
                    {
                        ...
                        NextSeq(20);
                        GbVar.Seq.sUldGDTrayTable[0].smTableUP = false;
                        return;
                    }
        ...
        bool IsStageUp()
        {
            ...
            if (GbVar.Seq.sUldGDTrayTable[0].smTableUP)
                return true;
#endif
            IODF.INPUT[] input = { IODF.INPUT.GD_TRAY_1_STG_UP,
                                    IODF.INPUT.GD_TRAY_2_STG_UP };
