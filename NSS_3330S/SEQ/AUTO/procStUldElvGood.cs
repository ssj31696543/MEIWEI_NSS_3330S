    ...
    public class procStUldElvGood : SeqBase
    {
        ...
        public override void Run()
        {
            ...
            switch ((seqUldTrayElvGood.SEQ_SOTER_ULD_ELV_GOOD)m_nSeqNo)
            {
                ...
                case seqUldTrayElvGood.SEQ_SOTER_ULD_ELV_GOOD.WAIT_TRAY_IN_TRANSFER:
                    {
                        ...
                        if (GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].bSeqIfVar[seqUldTrayElvGood.LOT_END_CHECKED] ||
                            GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].nTrayInCount >= ConfigMgr.Inst.Cfg.itemOptions[(int)OPTNUM.ULD_GD_TRAY_ELV_1_OUT_MAX_COUNT + m_nElvPorNum].nValue ||//220824 pjh
                            GbVar.bTrayOutReq[m_nElvPorNum] == true)
                        {
                            ...
                            if (!GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].bSeqIfVar[seqUldTrayElvGood.TRAY_TRANSFER_PLACE_PROCESS])
                            {
                                ...
                                {
                                    if (GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].bSeqIfVar[seqUldTrayElvGood.LOT_END_CHECKED]
                                        && (GbVar.Seq.sUldGDTrayTable[0].bTrayExist || GbVar.Seq.sUldGDTrayTable[1].bTrayExist))
                                        return;

                                    if (GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].nTrayInCount == 0)
                                    {
                                        NextSeq(seqUldTrayElvGood.SEQ_SOTER_ULD_ELV_GOOD.FINISH);
                                        return;
                                    }

                                    GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].bSeqIfVar[seqUldTrayElvGood.TRAY_IN_READY] = false;
                                    GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].bSeqIfVar[seqUldTrayElvGood.TRAY_COVER_READY] = true;
                                    SeqHistory("Lot End Checked", String.Format("Good {0} Tray Elevator", m_nElvPorNum + 1), "Checked");
                                    NextSeq(seqUldTrayElvGood.SEQ_SOTER_ULD_ELV_GOOD.WAIT_TRAY_COVER);
                                    return;
                                }
                                else
                                {
                        ...
                        if (!GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].bSeqIfVar[seqUldTrayElvGood.TRAY_IN_COMP])
                        {
                            if (GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].bSeqIfVar[seqUldTrayElvGood.TRAY_EXIST])
                            {
                                GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].bSeqIfVar[seqUldTrayElvGood.TRAY_EXIST] = false;
                                GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].bSeqIfVar[seqUldTrayElvGood.TRAY_IN_COMP] = true;
                            }
                            return;
                        }
                ...
                case seqUldTrayElvGood.SEQ_SOTER_ULD_ELV_GOOD.WAIT_TRAY_COVER:
                    if (LeaveCycle() == false) 
                        return;
                    ...
                    if (!GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].bSeqIfVar[seqUldTrayElvGood.TRAY_COVER_COMP])
                        return;

                    GbVar.Seq.sUldTrayElvGood[m_nElvPorNum].bSeqIfVar[seqUldTrayElvGood.TRAY_COVER_COMP] = false;
                ...
                case seqUldTrayElvGood.SEQ_SOTER_ULD_ELV_GOOD.TRAY_OUT:
                    if (m_bFirstSeqStep)
                    {
                        m_bFirstSeqStep = false;
                    }
