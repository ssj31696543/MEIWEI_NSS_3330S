...
{
    public  class procStUnitPnP2 : SeqBase
    {
        cycPkgPickNPlace cycFunc = null;

        int m_nMapTablePickUpCount;
        string m_strHead = "MULTI PICKER HEAD 2";
        ...
        int m_nRetry = 0;
        public procStUnitPnP2(int nSeqID)
        {
            m_nHeadNo = CFG_DF.HEAD_2;
            m_nSideHeadNo = CFG_DF.HEAD_1;
        ...
        public override void Run()
        {
            ...
            if (ConfigMgr.Inst.Cfg.itemOptions[(int)OPTNUM.PICKER_2_USE].bOptionUse == false)
            {
            ...
            switch ((seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE)m_nSeqNo)
            {
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.IS_OTHER_HEAD_PICK_UP_CYCLE_RUN_BEFORE_STANDBY_POS:
                    {
                            ...
                            GbVar.Seq.sMapVisionTable[1].bSeqIfVar[seqMapVisionTable.MAP_TABLE_UNLOAD_START])
                        {
                            ...
                            if ((ConfigMgr.Inst.Cfg.itemOptions[(int)OPTNUM.PICKER_1_USE].bOptionUse &&
                                !GbVar.Seq.sPkgPickNPlace.pInfo[m_nSideHeadNo].bSeqIfVar[seqPkgPickNPlace.SYNC_PICK_UP_READY]) ||
                                GbVar.Seq.sPkgPickNPlace.pInfo[CFG_DF.HEAD_1].bSeqIfVar[seqPkgPickNPlace.SYNC_PICK_UP_START] == true)
                            {
                                break;
                            }
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.MOVE_TO_PICKER_X_AXIS_PICK_UP_POS:
                    ...
                    if (m_bFirstSeqStep)
                    {
                        ...
                        if (GbVar.Seq.sPkgPickNPlace.pInfo[m_nSideHeadNo].bSeqIfVar[seqPkgPickNPlace.SYNC_PICK_UP_START])
                        {
                            ...
                            {
                                if (ConfigMgr.Inst.Cfg.itemOptions[(int)OPTNUM.PICKER_2_PAD_1_USE].bOptionUse == true)
                                {
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.IS_IN_STANDBY_POS:
                    {
                        // 축이 움직이는지 확인
                        if (MotionMgr.Inst[SVDF.AXES.CHIP_PK_2_X].IsAlarm())
                        {
                            SetError((int)ERDF.E_SV_AMP + (int)SVDF.AXES.CHIP_PK_2_X);
                            return;
                        }

                        if (MotionMgr.Inst[SVDF.AXES.CHIP_PK_2_X].GetServoOnOff() == false)
                        {
                            SetError((int)ERDF.E_SV_SERVO_ON + (int)SVDF.AXES.CHIP_PK_2_X);
                            return;
                        }

                        if (MotionMgr.Inst[SVDF.AXES.CHIP_PK_2_X].IsInPosition() == false ||
                            MotionMgr.Inst[SVDF.AXES.CHIP_PK_2_X].IsBusy())
                        {
                            return;
                        }
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.IS_OTHER_HEAD_PICK_UP_CYCLE_RUN:
                    ...
                    if ((ConfigMgr.Inst.Cfg.itemOptions[(int)OPTNUM.PICKER_1_USE].bOptionUse && 
                        !GbVar.Seq.sPkgPickNPlace.pInfo[m_nSideHeadNo].bSeqIfVar[seqPkgPickNPlace.SYNC_PICK_UP_READY]) ||
                        GbVar.Seq.sPkgPickNPlace.pInfo[CFG_DF.HEAD_1].bSeqIfVar[seqPkgPickNPlace.SYNC_PICK_UP_START] == true) return;
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.IS_MAP_TABLE_INSPECTION_COMPLETED:
                    ...
                    // PnP Head #1에서만 소스작업
                    // [2022.06.12.kmlee] 1번 Skip 후 다시 풀면 2번은 pickup start 상태이고 테이블 선택이 안된 상태여서 무언정지 됨
                    //if (ConfigMgr.Inst.Cfg.itemOptions[(int)OPTNUM.PICKER_1_USE].bOptionUse == true)
                    //{
                    //    if (GbVar.Seq.sMapVisionTable[cycFunc.CHIP_PICKER_PICK_UP_TABLE_NO].bSeqIfVar[seqMapVisionTable.MAP_TABLE_UNLOAD_READY] == false) return;
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.IS_F_MAP_TABLE_ARRAY_COUNT_OVER:
                        ...
                        cycFunc.CHIP_PICKER_PICK_UP_TABLE_COUNT != 0)
                    {
                        ...
                        if (cycFunc.CHIP_PICK_UP_FWD_COUNT + cycFunc.CHIP_PICK_UP_SET_BWD_COUNT > 0)
                        {
                            ...
                            GbVar.Seq.sMapVisionTable[cycFunc.CHIP_PICKER_PICK_UP_TABLE_NO].bSeqIfVar[17] = true;
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.IS_F_MAP_PICK_UP_UNIT_EMPTY:
                    ...
                    {
                        // 여기서 무한루프 디버깅 필요!!! 20211117 bhoh ToDo :
                        cycFunc.CHIP_PICKER_PICK_UP_COL_COUNT++;
                        //cycFunc.CHIP_PICKER_PICK_UP_TABLE_COUNT++;
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.CYC_F_UNIT_PICK_UP:
                    {
                        ...
                        if (nFuncResult == FNC.SUCCESS)
                        {
	                        ...
	                        GbVar.lstBinding_EqpProc[MCDF.CURRLOT].MULTI_PICKER_2_PICKUP_COUNT++;
                            //배출할때 체크
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.BALL_INSP_INTF_INIT_REQ:
                    {
                        IFMgr.Inst.VISION.SetBallHeadNo(MCDF.eUNIT.NO2);
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.MOVE_TO_ALL_XYZ_AXIS_VISION_POS:
                    {
                       // nFuncResult = MovePosBallVisionXYZ(POSDF.BALL_VISION_CHIP_BGA_INSPECTION_HEAD_1 + m_nHeadNo, m_nHeadNo);
                        ...
                        nFuncResult = MovePosBallVisionXYZT(POSDF.BALL_VISION_CHIP_BGA_INSPECTION_HEAD_1 + m_nHeadNo, m_nHeadNo); //sj.shin 칭허 적용
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.REQ_BTM_INSPECTION_COMPLETE:
                    {
                        ...
                        if (!IFMgr.Inst.VISION.IsBallInspCompleted) return;
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.IS_GOOD_PLACE_SKIP:
                    {
                        if (IsPickerPadSkip(false, CFG_DF.HEAD_2, cycFunc.CHIP_PLACE_SET_BWD_COUNT))
                        {
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.CYC_GOOD_UNIT_PLACE:
                    {
                        ...
                        if (nFuncResult == FNC.SUCCESS)
                        {
                            ...
                            GbVar.lstBinding_EqpProc[MCDF.CURRLOT].MULTI_PICKER_2_PLACE_COUNT++;
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.THROW_OUT_TO_REJECT_BOX_1:
                    ...
                    {
                        if (GbVar.GB_CHIP_PK_VAC[CFG_DF.MAX_PICKER_PAD_CNT + nPadCnt])
                        {
                            MotionMgr.Inst.SetSigOutput((int)SVDF.AXES.CHIP_PK_2_Z_1 + (2 * nPadCnt), 2, false);
                            MotionMgr.Inst.SetSigOutput((int)SVDF.AXES.CHIP_PK_2_Z_1 + (2 * nPadCnt), 3, true);
                            ...
                            GbVar.product.nTotalCnt++;
                            GbVar.Seq.sMapVisionTable[cycFunc.CHIP_PICKER_PICK_UP_TABLE_NO].Info.X_MARK_UNIT++;
                            SetNGUnit(nPadCnt, 0);
                        }
                    }
                    break;
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.THROW_OUT_TO_REJECT_BOX_3:
                    for (int nPadCnt = 0; nPadCnt < CFG_DF.MAX_PICKER_PAD_CNT; nPadCnt++)
                    {
                        MotionMgr.Inst.SetSigOutput((int)SVDF.AXES.CHIP_PK_2_Z_1 + (2 * nPadCnt), 3, false);
                    }
                    break;
                ...
                case seqPkgPickNPlace.SEQ_SOTER_UNIT_PICK_N_PLACE.FINISH:
                    if (GbVar.Seq.sMapVisionTable[cycFunc.CHIP_PICKER_PICK_UP_TABLE_NO].bSeqIfVar[17] == true)
                    {
        ...
        public bool IsMapPickUpUnit(bool bFDir, int nMapTableNo, int nTablePickUpTotCount, int nTablePickUpColCount)
        {
            ...
            return GbVar.Seq.sMapVisionTable[nMapTableNo].Info.UnitArr[nRow][nCol].IS_UNIT;//;//220621            return GbVar.Seq.sMapVisionTable[nMapTableNo].Info.UnitArr[nRow][nCol].IS_UNIT;
        }
