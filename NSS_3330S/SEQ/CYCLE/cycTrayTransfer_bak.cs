    ...
    public class cycTrayTransfer :SeqBase
    {
        ...
        public int TrayLoadingFromElv()
        {
            ...
            switch (m_nSeqNo)
            {
                ...
                case 5:
                    if (MotionMgr.Inst.GetInput(IODF.INPUT.TRAY_PK_FRONT_UNCLAMP) && 
                        MotionMgr.Inst.GetInput(IODF.INPUT.TRAY_PK_REAR_UNCLAMP))
                    {
                        ...
                        MotionMgr.Inst.GetInput(IODF.INPUT.TRAY_PK_REAR_CLAMP))
                    {
                        if (MotionMgr.Inst.GetInput(IODF.INPUT.TRAY_PK_FRONT_TRAY_CHECK) ||
                            MotionMgr.Inst.GetInput(IODF.INPUT.TRAY_PK_REAR_TRAY_CHECK))
                        {
                            return (int)ERDF.E_TRAY_PICKER_FRONT_CLAMP_SENSOR_NOT_OFF;
                        }
                    }
                    break;
                ...
                case 14:
                    ...
                    {
                        nFuncResult = MovePosTrayPkXY(POSDF.TRAY_PICKER_TRAY_ELV_EMPTY_1 + LOAD_ELV_NO);

                        if (FNC.IsSuccess(nFuncResult))
                        {
                            SeqHistory(string.Format("ELAPSED, {0}, {1}", "TRAY PICKER XY AXIS LOAD POSITION COMPLETE", STEP_ELAPSED));
                        }
                    }
                    break;
            ...
            switch (m_nSeqNo)
            {
                ...
                case 5:
                        ...
                        MotionMgr.Inst.GetInput(IODF.INPUT.TRAY_PK_REAR_UNCLAMP))
                    {
                        ...
                        if (LOAD_TABLE_NO == 1)
                        {
                            if (MotionMgr.Inst.GetInput(IODF.INPUT.GD_TRAY_2_CHECK) == false)
                            {
                                NextSeq(40);
                                return FNC.BUSY;
                            }
                        ...
                        if (LOAD_TABLE_NO == 2)
                        {
                            if (MotionMgr.Inst.GetInput(IODF.INPUT.RW_TRAY_CHECK) == false)
                            {
                                NextSeq(40);
                                return FNC.BUSY;
                            }
                    ...
                    else
                    {
                            ...
                            MotionMgr.Inst.GetInput(IODF.INPUT.TRAY_PK_REAR_CLAMP))
                        {
                            ...
                            else
                            {
                                ...
                                //2가지 경우
                                //다 아니면 알람
                                SeqHistory(string.Format("ELAPSED, {0}, {1}", "TRAY CHECK SENSOR NOT ON", STEP_ELAPSED));
                                return (int)ERDF.E_TRAY_PK_GRIP_CHECK_FAIL;
                            }
                ...
                case 17:
                    // TRAY TRANSFER XY EMPTY + n POS MOVE (0: EMPTY1, 1: EMPTY2, 2: COVER)
                    {
                        nFuncResult = MovePosTrayPkXY(POSDF.TRAY_PICKER_STAGE_GOOD_1 + LOAD_TABLE_NO);
                ...
                case 40:
                    {
                        if (m_bFirstSeqStep)
                        {
                            if (IsInPosTrayPkZ(POSDF.TRAY_PICKER_READY))
                            {
                                break;
                            }
                        }
        ...
        public int TrayUnloadingToElv()
        {
            ...
            switch (m_nSeqNo)
            {
                ...
                case 20:
                    ...
                    {
                        nFuncResult = MovePosTrayPkXY(POSDF.TRAY_PICKER_TRAY_ELV_GOOD_1 + UNLOAD_ELV_NO);
        ...
        public int TrayUnloadingToEmptyElv()
        {
            ...
            switch (m_nSeqNo)
            {
                ...
                case 20:
                    ...
                    {
                        nFuncResult = MovePosTrayPkXY(POSDF.TRAY_PICKER_TRAY_ELV_EMPTY_1 + UNLOAD_ELV_NO);
        ...
        public int TrayUnloadingToTable()
        {
            ...
            switch (m_nSeqNo)
            {
                ...
                case 20:
                    ...
                    {
                        nFuncResult = MovePosTrayPkXY(POSDF.TRAY_PICKER_STAGE_GOOD_1 + UNLOAD_TABLE_NO);
