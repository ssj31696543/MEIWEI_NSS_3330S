...
namespace NSS_3330S.SEQ.CYCLE
{
    public class cycUldTrayTable: SeqBase
    {
        ...
        #region Manual Operating Function
        public int GoodTrayMove()
        {
            if (m_nSeqNo != m_nPreSeqNo)
            {
                ResetCmd();
                ...
                case 10:
                    {
                        ...
                        nFuncResult = MovePosGdTrayStgY(nOtherTrayTableNo, POSDF.TRAY_STAGE_TRAY_UNLOADING);
                ...
                case 18:
                    {
                        ...
                        nFuncResult = MovePosGdTrayStgY(m_nTableNo, m_dTargetPos, ConfigMgr.Inst.Cfg.MotData[(int)SVDF.AXES.GD_TRAY_STG_1_Y + m_nTableNo].dVel);
        ...
        public int GoodTrayMove(int nAxisNo, double dPos)
        {
            if (m_nSeqNo != m_nPreSeqNo)
            {
                ResetCmd();
        ...
        int MovePosGdTrayStgY(int nTableNo, double dPos, double dVel, long lDelay = 0)
        {
            ...
            for (int nCnt = 0; nCnt < nAxisArray.Length; nCnt++)
            {
                ...
                dAccArray[nCnt] = dVel * 10;
                dDecArray[nCnt] = dVel * 10;
            }
            ...
            if (RunLib.IsCurRunCmdStart())
            {
                ...
                for (int nCnt = 0; nCnt < nAxisArray.Length; nCnt++)
                {
                    m_strMotPos = string.Format("{0}[{1} - {2}]", m_strMotPos,
