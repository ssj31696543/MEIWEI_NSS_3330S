...
{
    public class cycLoader_UldConv : SeqBase
    {
        ...
        public cycLoader_UldConv(int nSeqID)
        {
            SetCycleMode(true);
                        ...
                        else
                        {
                            if (!IsUldConv_Arrival(0))
                            {
                                    ...
                                    IsDelayOver(ConfigMgr.Inst.Cfg.itemOptions[(int)OPTNUM.CONV_MAX_ROLLING_TIMEOUT].lValue))
                                {
                                    ...
                                    return (int)ERDF.E_MGZ_LD_CONV_1_LD_MGZ_ARRIVAL_FAIL + m_ConvNo;
                                }
                                return FNC.BUSY;
                            }
                        }
        ...
        public int MzUnloadingRun_1(bool bSensorSkip = false)
        {
            if (m_nSeqNo != m_nPreSeqNo)
            {
                ResetCmd();
        ...
        public int MzUnloadingRun_2(bool bSensorSkip = false)
        {
            if (m_nSeqNo != m_nPreSeqNo)
            {
                ResetCmd();
        ...
        public int MgzUldConvUloading_Wait()
        {
            try
            {
                if (m_nSeqNo != m_nPreSeqNo)
                ...
                switch (m_nSeqNo)
                {
                    ...
                    case 4:
                        {
                            ...
                            TTDF.SetTact((int)TTDF.CYCLE_NAME.MGZ_UNLOADING_WAIT, true);
                        }
                        break;
                    ...
                    case 6:
                        ...
                        if (GbVar.Seq.sLoaderUldConv.bSeqIfVar[seqLoader_UldConv.MGZ_CONV_RUN]
                            || GbVar.Seq.sLdMzLoading.bSeqIfVar[seqLdMzLoading.MGZ_LOAD_CONV_UNLOADING_REQ])
                        {
                            m_bSafetyConvStart = true;
                            m_bSafetyConvStop = false;
                            // 컨베이어 동작 상태면 대기
                    ...
                    case 20:
                        {
                            TTDF.SetTact((int)TTDF.CYCLE_NAME.MGZ_UNLOADING_WAIT, false);
